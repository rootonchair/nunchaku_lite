...
DEFAULT_PROMPT = (
    "a cinematic photo of a glass greenhouse full of tropical plants during golden hour, "
    "detailed, natural light"
)
...
def run_lite(args: argparse.Namespace, output_dir: Path, pipeline_cls, torch_dtype: torch.dtype) -> dict:
    ...
    cleanup()
    print("loading base pipeline for nunchaku_lite", flush=True)
    pipe, load_seconds = timed_cuda_call(
        lambda: pipeline_cls.from_pretrained(
            args.model_id,
            torch_dtype=torch_dtype,
    ...
    print("patching transformer with nunchaku_lite", flush=True)
...
def main() -> None:
    ...
    if "original_diffusers" in results and "nunchaku_lite" in results:
        original_mean = results["original_diffusers"]["seconds"]["mean"]
        lite_mean = results["nunchaku_lite"]["seconds"]["mean"]
        results["speedup"] = original_mean / lite_mean
